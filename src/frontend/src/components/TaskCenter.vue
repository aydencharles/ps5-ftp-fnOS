...
const activeTab = computed(() => tasks.centerTab)
...
const launcherProgress = computed(() => {
  const measurable = activeTasks.value.filter((task) => task.total_bytes > 0)
  if (!measurable.length) return 0
  const total = measurable.reduce((sum, task) => sum + task.total_bytes, 0)
  const transferred = measurable.reduce((sum, task) => sum + Math.min(task.transferred_bytes, task.total_bytes), 0)
  return total ? Math.round(transferred / total * 100) : 0
})
...
const launcherLabel = computed(() => activeTasks.value.length
  ? `任务中心，${activeTasks.value.length} 个任务进行中，整体进度 ${launcherProgress.value}%`
  : '任务中心，没有正在进行的任务')
...
function launcherCoordinates() {
  ...
  const { edge, ratio } = tasks.launcherPosition
...
function updatePanelPosition() {
  ...
  if (tasks.launcherPosition.edge === 'left') x = launcherPoint.x + launcherSize + gap
  if (tasks.launcherPosition.edge === 'right') x = launcherPoint.x - panelWidth - gap
  if (tasks.launcherPosition.edge === 'top') y = launcherPoint.y + launcherSize + gap
  if (tasks.launcherPosition.edge === 'bottom') y = launcherPoint.y - panelHeight - gap
...
function snapLauncherToNearestEdge() {
  ...
  tasks.setLauncherPosition({ edge, ratio: clamp((coordinate - launcherMargin) / usable, 0, 1) })
  drag.active = false
...
function toggleCenter() {
  ...
  if (tasks.centerVisible) tasks.closeCenter()
  else tasks.openCenter(activeTasks.value.length ? 'active' : 'history')
}
...
function handleKeydown(event: KeyboardEvent) {
  if (event.key === 'Escape' && tasks.centerVisible && !detailVisible.value) closeCenter()
}
...
tasks.hydrateLauncherPosition()
refreshLauncherPosition()

watch([() => tasks.centerVisible, () => tasks.centerTab, () => visibleTasks.value.length], async ([visible]) => {
  if (!visible) return
  await nextTick()
  updatePanelPosition()
})
...
<template>
  <Teleport to="body">
    <Transition name="task-center-fade">
      <button v-if="tasks.centerVisible" class="task-center-scrim" aria-label="关闭任务中心" @click="closeCenter" />
    </Transition>

    <Transition name="task-center-panel">
      <aside v-if="tasks.centerVisible" id="task-center-panel" ref="panelElement" class="task-center-panel" :class="`is-docked-${tasks.launcherPosition.edge}`" :style="panelStyle" role="dialog" aria-label="任务中心">
        <header class="task-center-header">
          <div class="task-center-heading">
            <span class="task-center-heading-icon"><ListTodo :size="19" /></span>
            <div>
              <h2>任务中心</h2>
              <p v-if="activeTasks.length">{{ activeTasks.length }} 个任务正在处理 · 整体 {{ launcherProgress }}%</p>
              <p v-else>传输与文件操作记录</p>
            </div>
          </div>
          <button class="task-icon-button task-center-close" aria-label="关闭任务中心" @click="closeCenter"><X :size="18" /></button>
        </header>

        <div class="task-center-tabs" role="tablist" aria-label="任务分类">
          <button id="task-tab-active" role="tab" :aria-selected="activeTab === 'active'" aria-controls="task-center-list" :class="{ 'is-active': activeTab === 'active' }" @click="switchTab('active')">
            <ListTodo :size="15" /><span>进行中</span><em>{{ activeTasks.length }}</em>
          </button>
          <button id="task-tab-history" role="tab" :aria-selected="activeTab === 'history'" aria-controls="task-center-list" :class="{ 'is-active': activeTab === 'history' }" @click="switchTab('history')">
            <History :size="15" /><span>历史任务</span><em>{{ historyTasks.length }}</em>
          </button>
        </div>

        <div id="task-center-list" class="task-center-list" role="tabpanel" :aria-labelledby="`task-tab-${activeTab}`">
          <article v-for="task in visibleTasks" :key="task.id" class="task-center-item" :class="[`is-${task.state}`, { 'is-active-task': activeTab === 'active' }]">
            <header class="task-item-header">
              <span class="task-state-chip"><i :class="['state-mark', `state-${task.state}`]" />{{ stateLabel[task.state] || task.state }}</span>
              <span class="task-item-id">#{{ task.id.slice(0, 8) }}</span>
            </header>

            <div class="task-center-route" :title="`${sourceLabel(task)} → ${destinationLabel(task)}`">
              <span><small>从</small>{{ sourceLabel(task) }}</span>
              <i>→</i>
              <span><small>{{ task.type === 'delete' ? '操作' : '到' }}</small>{{ destinationLabel(task) }}</span>
            </div>

            <template v-if="activeTab === 'active'">
              <p class="task-center-subtitle" :class="{ 'is-error': Boolean(task.error) }">{{ taskSubtitle(task) }}</p>
              <div class="task-center-progress-row">
                <div class="task-center-progress"><i :style="{ width: `${percent(task)}%` }" /></div>
                <strong>{{ Math.round(percent(task)) }}%</strong>
              </div>
              <div class="task-center-metrics">
                <span><strong>{{ speedLabel(task) }}</strong><small>{{ etaLabel(task) }}</small></span>
                <span><strong>{{ formatBytes(task.transferred_bytes) }}</strong><small>共 {{ formatBytes(task.total_bytes) }}</small></span>
              </div>
              <footer class="task-center-actions">
                <button @click="openDetail(task)"><Eye :size="14" />详情</button>
                <button class="is-danger" :disabled="task.state === 'canceling'" @click="cancel(task)"><Ban :size="14" />{{ task.state === 'canceling' ? '取消中' : '取消' }}</button>
              </footer>
            </template>
            ...
            <template v-else>
              ...
              <div class="task-history-meta">
                <span><Clock3 :size="13" />{{ formatTime(task.created_at) }}</span>
                <strong>{{ historyDataLabel(task) }}</strong>
              </div>
              <footer class="task-center-actions">
                <button @click="openDetail(task)"><Eye :size="14" />详情</button>
                <button v-if="['failed', 'canceled', 'interrupted'].includes(task.state)" @click="retry(task)"><RotateCcw :size="14" />重新提交</button>
                <button class="is-danger is-icon-only" aria-label="删除任务记录" title="删除任务记录" @click="removeHistory(task)"><Trash2 :size="14" /></button>
              </footer>
            </template>
          </article>

          <div v-if="!visibleTasks.length" class="task-center-empty">
            <CircleCheck v-if="activeTab === 'active'" :size="34" />
            <History v-else :size="34" />
            <strong>{{ activeTab === 'active' ? '当前没有进行中的任务' : '还没有历史任务' }}</strong>
            <span>{{ activeTab === 'active' ? '新建传输后，进度会显示在这里' : '完成、失败或取消的任务会保留在这里' }}</span>
          </div>
        </div>

        <footer class="task-center-note"><Info :size="13" /><span>同一台 PS5 一次执行一个任务，多台设备可并行</span></footer>
      </aside>
    </Transition>

    <div class="task-launcher-ring" :class="{ 'has-active': activeTasks.length, 'is-dragging': drag.active }" :style="launcherStyle">
      <button class="task-launcher" :aria-label="`${launcherLabel}；长按可移动`" :title="`${launcherLabel}；长按可移动`" aria-controls="task-center-panel" :aria-expanded="tasks.centerVisible" @click="toggleCenter" @pointerdown="handlePointerDown" @pointermove="handlePointerMove" @pointerup="finishPointerInteraction" @pointercancel="finishPointerInteraction" @contextmenu.prevent>
        <X v-if="tasks.centerVisible" :size="21" />
        <ListTodo v-else :size="22" />
      </button>
      <span v-if="activeTasks.length" class="task-launcher-badge" aria-hidden="true">{{ activeTasks.length > 99 ? '99+' : activeTasks.length }}</span>
    </div>
  </Teleport>
  ...
  <t-dialog v-model:visible="detailVisible" header="任务详情" width="720px" :footer="false">
    <div v-if="detailTask" class="task-detail" :class="{ 'is-loading': detailLoading }">
      ...
      <dl class="task-detail-grid">
        ...
        <div><dt>冲突策略</dt><dd>{{ detailTask.type === 'delete' ? '—' : conflictLabel[detailTask.conflict_policy] || '—' }}</dd></div>
        ...
        <div><dt>创建时间</dt><dd>{{ formatTime(detailTask.created_at) }}</dd></div>
