...
export const useProfilesStore = defineStore('profiles', {
  ...
  actions: {
    hydrate(items: Profile[] | null) {
      this.items = items || []
      if (!this.items.some((profile) => profile.id === this.selectedId)) this.selectedId = this.items[0]?.id || ''
    },
    ...
    async save(profile: Partial<Profile>) {
      ...
      delete payload.id
      await api(`/api/v1/profiles${suffix}`, { method, body: JSON.stringify(payload) }); await this.refresh()
    },
