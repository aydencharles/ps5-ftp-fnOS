...
export const useTasksStore = defineStore('tasks', {
  ...
  actions: {
    ...
    async create(profileId: string, sources: SourceLocator[], destination: string, conflict: string) { const data = await api<{ task: Task }>('/api/v1/tasks', { method: 'POST', body: JSON.stringify({ profile_id: profileId, sources, destination, conflict_policy: conflict, type: 'upload' }) }); await this.refresh(); useTaskCenterStore().open('transfer', 'active'); return data.task },
    async createDownload(profileId: string, remotePaths: string[], destination: SourceLocator, conflict: string) {
      ...
      await this.refresh()
      useTaskCenterStore().open('transfer', 'active')
      return data.task
    },
