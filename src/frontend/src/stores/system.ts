...
import { api } from '../api'
import type { LibraryRoot, Profile, Task } from '../types'

export const useSystemStore = defineStore('system', {
  state: () => ({ ready: false, error: '', version: '', transferWorkers: 2 }),
  actions: {
    async bootstrap() {
      const data = await api<{ version: string; profiles: Profile[]; library_roots: LibraryRoot[]; tasks: Task[]; settings: { transfer_workers: number } }>('/api/v1/bootstrap')
