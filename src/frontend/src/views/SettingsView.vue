...
async function test(id: string) {
  try { await profiles.test(id); await MessagePlugin.success('连接成功，目录列表可用') }
  catch (error) { await MessagePlugin.error(error instanceof Error ? error.message : String(error)) }
}
</script>
      ...
      <div class="settings-body">
        ...
        <div v-if="profiles.items.length" class="profile-list">
          <div v-for="profile in profiles.items" :key="profile.id" class="profile-row">
            ...
            <div class="row-actions"><t-button variant="text" size="small" @click="test(profile.id)"><PlugZap :size="14" />测试连接</t-button><t-button variant="text" size="small" @click="edit(profile)"><Pencil :size="14" />编辑</t-button><t-button variant="text" theme="danger" size="small" @click="remove(profile)"><Trash2 :size="14" />删除</t-button></div>
          </div>
        </div>
